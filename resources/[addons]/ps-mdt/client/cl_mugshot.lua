...
RegisterNUICallback("sendToJail", function(data, cb)
    local citizenId, caseId, sentence, charges = data.citizenId, data.caseId, data.sentence, data.charges
    ...
    if sentence > 0 then
        ...
        TriggerServerEvent("mdt:server:sentencePlayer", citizenId, caseId, sentence, charges)
    end
end)
