...
RegisterNetEvent('esx:playerLoaded', function()
    ...
    ESX.TriggerServerCallback("ps-mdt:getSelfData", function (data) 
...
RegisterCommand('mdt', function()
    ...
    if not exports.wasabi_ambulance:isPlayerDead(player.source) and not IsPauseMenuActive() then
        if GetJobType(player.job.name) ~= nil then
        ...
        lib.notify({
            type = "error",
            description = "You do not have permissions for that."
        })
    end
end, false)
...
local function EnableGUI(enable)
    ...
    SendNUIMessage({ type = "show", enable = enable, job = ESX.PlayerData.job.name, jobGrade = ESX.PlayerData.job.grade, jobType = ESX.PlayerData.job.type, rosterLink = Config.RosterLink[ESX.PlayerData.job.name], sopLink = Config.sopLink[ESX.PlayerData.job.name] })
    isOpen = enable
    doAnimation()
end
...
local function RefreshGUI()
    ...
    SendNUIMessage({ type = "show", enable = false, job = ESX.PlayerData.job.name, jobGrade = ESX.PlayerData.job.grade, jobType = ESX.PlayerData.job.type, rosterLink = Config.RosterLink[ESX.PlayerData.job.name], sopLink = Config.sopLink[ESX.PlayerData.job.name] })
    isOpen = false
end
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits, calls, cid)
    ...
    SendNUIMessage({ 
        ...
        name = "Welcome, " ..player.job.grade_label..' '..player.firstName:sub(1,1):upper()..player.lastName:sub(2), 
        location = playerStreetsLocation, 
        fullname = player.name, 
        bulletin = bulletin,
        gangwar = GlobalState.gangWar,
        cooldown = GlobalState.cooldown,
        forcedcooldown = GlobalState.forcedcooldown
    })
...
RegisterNUICallback("getProfileData", function(data, cb)
    ...
    for i=1,#vehicles do
        ...
        local vehData = exports.stem_dealership:getVehicle(vehicle.model)
        ...
        if vehData == nil then
            ...
            result.vehicles[i]['model'] = vehData.label
        end
    end
    p = nil
...
RegisterNUICallback("sendFine", function(data, cb)
    ...
    if fine > 0 then
        TriggerServerEvent("mdt:server:finePlayer", citizenId, fine, incidentId)
    end
end)
    ...
    if not isLimited then
        ...
        for i=1, #vehicles do
            ...
            if tempModel and tempModel ~= "Unknown" then
                local vehData = exports.stem_dealership:getVehicle(tempModel)
                sentData['vehicles'][i]['model'] = vehData.label
            end
        end
    end
