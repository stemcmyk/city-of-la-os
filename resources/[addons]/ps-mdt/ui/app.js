...
const activeJobs = {
  "sahp": { color: "#cc7e25", type: "police" },
  "lscso": { color: "#5acc25", type: "police" },
  "lspd": { color: "#2554cc", type: "police" },
  "amr": { color: "#950909", type: "ems" },
}
...
$(document).ready(() => {
  ...
  $(".profile-items").on("click", ".profile-item", async function () {
    ...
    if (licenses.length > 0 && (activeJobs[playerJob] !== undefined)) {
      licencesHTML = '';
  ...
  $('.incidents-ghost-holder').on('click', '#jail-button', function () {
    ...
    sendToJail(citizenId, dbid, sentence, recommendSentence, charges);
  });
        ...
        $(".incidents-ghost-holder").prepend(
          `
            <div class="associated-incidents-user-container" data-id="${$(this).data("cid")}">
                ...
                <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Total Fine</div>
                ...
                <div class="associated-incidents-controls" data-id="${$(this).data("cid")}">
                    <div id="jail-button" class="control-button" data-id="${$(this).data("cid")}"><span class="fa-solid fa-building-columns" style="margin-top: 3.5px;"></span> Sentence</div>
...
`;
  ...
  function JobColors(sentJob) {
    if (sentJob) {
      if (activeJobs[sentJob] !== undefined) {
        if (sentJob == "police") {
  ...
  window.addEventListener("message", function (event) {
    ...
    if (eventData.type == "show") {
      if (eventData.enable == true) {
        ...
        playerJobGrade = eventData.grade;
        PlayerJobType = eventData.jobType;

        JobColors(playerJob);
        $(".quote-span").html(randomizeQuote());
        if (activeJobs[playerJob] !== undefined) {
      ...
      let cooldown = eventData.cooldown
      let forcedCooldown = eventData.forcedcooldown
      let gangwar = eventData.gangwar

      if (cooldown) {
        if (cooldown["sahp"]) {
          $("#status-county-cooldown .status-data").html(`Active: ${cooldown["sahp"]}m`)
          $("#status-county-cooldown").addClass("status-active")
        } else {
          $("#status-county-cooldown .status-data").html("Inactive")
          $("#status-county-cooldown").removeClass("status-active")
        }

        if (cooldown["lspd"]) {
          $("#status-city-cooldown .status-data").html(`Active: ${cooldown["lspd"]}m`)
          $("#status-city-cooldown").addClass("status-active")
        } else {
          $("#status-city-cooldown .status-data").html("Inactive")
          $("#status-city-cooldown").removeClass("status-active")
        }
      }

      if (gangwar) {
        $("#status-gangwar .status-data").html("Active")
        $("#status-gangwar").addClass("status-active")
      } else {
        $("#status-gangwar .status-data").html("Inactive")
        $("#status-gangwar").removeClass("status-active")
      }
      if (forcedCooldown) {
        $("#status-forced-cooldown .status-data").html("Active")
        $("#status-foeced-cooldown").addClass("status-active")
      } else {
        $("#status-city-cooldown .status-data").html("Inactive")
        $("#status-city-cooldown").removeClass("status-active")
      }
      ...
      activeUnits.forEach((unit) => {
        ...
        if (activeJobs[unit.unitType] !== undefined) {
          const jobInfo = activeJobs[unit.unitType]
          if (!jobCount[unit.unitType]) jobCount[unit.unitType] = 0;
          jobCount[unit.unitType]++
          activeInfoJob = `<div class="unit-job" style="background-color:${jobInfo.color}"}}>${unit.unitType.toUpperCase()}</div>`;
        }
        unitListHTML += `
      ...
      $("#job-count").append(Object.keys(activeJobs).map((key) => {
        const count = jobCount[key]
        const jobInfo = activeJobs[key]

        if ($(`#${key}-job-count`).length > 0) {
          return $(`#${key}-job-count`).html(`${count || 0}`)
        }
        return $(`<div class="active-unit-count" style="background-color:${jobInfo.color}" id="${key}-job-count">${count || 0}</div>`)
      }))
      ...
      $.each(table, function (index, value) {
        if (value && value?.job?.includes(playerJob) || value?.jobs.includes(playerJob)) {
          DispatchMAP(value);
          const prio = value["priority"];
          let DispatchItem = `<div class="active-calls-item" data-id="${value.callId || value.id}" data-canrespond="false"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#${value.callId || value.id}</div><div class="call-code priority-${value.priority}">${value.dispatchCode || value.code}</div><div class="call-title">${value.dispatchMessage || value.message}</div><div class="call-radio">${value.units.length}</div></div><div class="call-item-bottom">`;
      ...
      $(".manage-incidents-reports-content").trumbowyg('html', `<div style="color: white;"
        ${table["details"]}
      </div>`);
      ...
      $.each(associateddata, function (index, value) {
        ...
        const associatedIncidentsContainer = (value.associated != 1) && `
          ...
          <div class="associated-incidents-controls" data-id="${cid}">
            <div id="jail-button" class="control-button" data-id="${cid}"><span class="fa-solid fa-building-columns" style="margin-top: 3.5px;"></span> Sentence</div>
...
function sendToJail(citizenId, caseId, customSentence, recommendedSentence, charges) {
...
function searchProfilesResults(result) {
  ...
  result.forEach((value) => {
    ...
    if (licArr.length > 0 && (activeJobs[playerJob] !== undefined)) {
