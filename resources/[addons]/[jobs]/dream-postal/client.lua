...
SetBlipDisplay(blip, ESX.PlayerData?.job?.name ~= WHITELISTED_JOB_TITLE and 0 or GO_POSTAL_HQ_BLIP.display)
...
RegisterNetEvent("dream-postal:showBlip", function ()
    SetBlipDisplay(blip, GO_POSTAL_HQ_BLIP.display)
end)

RegisterNetEvent("dream-postal:hideBlip", function ()
    SetBlipDisplay(blip, 0)
end)


RegisterNetEvent("esx:setJob", function(newJob, lastJob)
    print(newJob.name, lastJob.name)
    if (newJob.name == lastJob.name) then return end
    if (newJob.name == WHITELISTED_JOB_TITLE) then
        TriggerEvent("dream-postal:showBlip")
        return
    end
    TriggerEvent("dream-postal:hideBlip")
end)
...
CreateThread(function()
    ...
    if (ESX.PlayerData.job.name ~= WHITELISTED_JOB_TITLE) then return end;
    TriggerEvent("dream-postal:showBlip")
end)
