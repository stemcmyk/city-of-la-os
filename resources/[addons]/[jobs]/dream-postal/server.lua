...
AddEventHandler("esx:setJob", function(source, newJob, lastJob)
    TriggerClientEvent("esx:setJob", source, newJob, lastJob)
end)
            ...
            exports.stem_discord:getUserDiscord(xPlayer.source, function (playerDiscord) 
                exports.stem_discord:sendThroughWebhook({
                    ...
                    params = {
                        ...
                        content = ([[
                            <@%s> ``%s (%s)`` **%s** finished a Go Postal route and made **$%s**
                        ]]):format(
                            playerDiscord, xPlayer.name, GetPlayerName(xPlayer.source), xPlayer.identifier, compensation
                        ),
                    },
                    ignoreEnv = true,
                })
