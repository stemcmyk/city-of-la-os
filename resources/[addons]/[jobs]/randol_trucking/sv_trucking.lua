...
lib.callback.register('randol_trucking:server:updateRoute', function(source, netid, route)
    ...
    if spawnedTrailers[src] == entity and route.index == data.currentRoute.index then
        local payment = data.currentRoute.payment
        AddMoney(player, 'bank', payment)
        DoNotification(src, ('You finished the route and received $%s'):format(payment), 'success', 7000)
            ...
            exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
                exports.stem_discord:sendThroughWebhook({
                    ...
                    params = {
                        ...
                        content = ([[
                            <@%s> ``%s (%s)`` **%s** finished a Trucking route and made **$%s**
                        ]]):format(
                            playerDiscord, player.name, GetPlayerName(player.source), player.identifier, payment
                        ),
                    },
                    ignoreEnv = true,
                })
