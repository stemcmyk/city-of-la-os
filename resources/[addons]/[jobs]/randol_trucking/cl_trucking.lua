...
SetBlipDisplay(TruckerWork, getJob() ~= Config.whitelistedJob and 0 or 4)
...
RegisterNetEvent("randol_trucking:showBlip", function ()
    SetBlipDisplay(TruckerWork, 4)
end)

RegisterNetEvent("randol_trucking:hideBlip", function ()
    SetBlipDisplay(TruckerWork, 0)
end)
...
RegisterNetEvent("esx:setJob", function(newJob, lastJob)
    ...
    if (newJob.name ~= Config.whitelistedJob) then
        TriggerEvent("randol_trucking:hideBlip")
        return
    end
    if (newJob.name == Config.whitelistedJob) then
        TriggerEvent("randol_trucking:showBlip")
    end
end)

CreateThread(function()
    while not hasPlyLoaded() do
        Wait(100)
    end
    if (getJob() ~= Config.whitelistedJob) then return end;
    TriggerEvent("randol_trucking:showBlip")
end)
