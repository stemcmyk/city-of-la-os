...
AddEventHandler('s_fishing:caught', function()
    ...
    if (not Player(xPlayer.source).state.isFishing) then
        return TriggerEvent("stem_ac:player:sus", {
			source = xPlayer.source, 
			event = "s_fishing:caught",
			reason = "Player doesn't have fishing state but still caught fish",
		})
    end
    ...
    if xPlayer ~=nil then 
        ...
        Player(xPlayer.source).state.isFishing = false
    end
end)
...
RegisterNetEvent("s_fishing:start", function () 
    Player(source).state.isFishing = true
end)
RegisterNetEvent("s_fishing:stop", function () 
    Player(source).state.isFishing = false
end)