...
local function OfficerDown()
    ...
    print(lastPanic and GetGameTimer() - lastPanic)
    if (lastPanic and (GetGameTimer() - lastPanic) < 3000) then
        return
    end

    if (not exports.stem_common:tableContains({ "amr", "lspd", "sahp", "lscso" }, PlayerData.job.name )) then
        return
    end
    lastPanic = GetGameTimer()
    
    local dispatchData = {
        message = locale('panicbutton'),
        codeName = 'panicbutton',
        code = '10-78',
...
RegisterCommand('panic', OfficerDown)
RegisterKeyMapping("panic", "LEO Panic Button", "keyboard", "F12")
...
RegisterNetEvent('ps-dispatch:client:sendEmergencyMsg', function(data, type, anonymous)
    local jobs = { ['911'] = { 'lspd', "sahp", "lscso" }, ['311'] = { 'ems' } }
...
local function ATMHacking()
    ...
    TriggerServerEvent("ps-dispatch:server:notify" ,{
        codeName = "atmhacking", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        message = "ATM hacking reported",
        ...
        jobs = { "lspd", "sahp" , "lscso"}, -- type or jobs that will get the alerts
        borderCheck = { "lspd" },
    })
end
