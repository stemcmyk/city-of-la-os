...
local function setupDispatch()
    local playerInfo = ESX.PlayerData
    local locales = lib.getLocales()
    PlayerData = ESX.PlayerData;
    ESX.TriggerServerCallback("ps-dispatch:getData", function (result) 
        PlayerData = {
            charinfo = {
            ...
            job = {
                ...
                label = playerInfo.job.grade_label
            },
        }
...
local function borderCheck(jobs, coords)
    local job = PlayerData.job.name
    if (not jobs) then return true end;
    if (table.contains(jobs, job)) then
        local isCoordsAboveBorder = coords.y > 500;
        local isPlayerAboveBoorder = GetEntityCoords(PlayerPedId()).y > 500
        if (isCoordsAboveBorder and isPlayerAboveBoorder) then
            return true
        end
        if (not isCoordsAboveBorder and not isPlayerAboveBoorder) then
            return true
        end
        return false
    end
...
local function createBlipData(coords, radius, display, sprite, color, alpha, scale, flash, shortRange)
...
local function createBlip(data, blipData)
    ...
    local display = blipData.display or 2
    local alpha = blipData.alpha or 255
    ...
    if blipData.offset then
        ...
        blip, radius = createBlipData({ x = offsetX, y = offsetY, z = data.coords.z }, blipData.radius, display, sprite, color, alpha, scale, flash, shortRange)
        blips[data.id] = blip
        radius2[data.id] = radius
    else
        blip, radius = createBlipData(data.coords, blipData.radius, display, sprite, color, alpha, scale, flash, shortRange)
        blips[data.id] = blip
        radius2[data.id] = radius
    end
...
RegisterNetEvent('ps-dispatch:client:notify', function(data, source)
    ...
    print(isJobValid(data.jobs), IsOnDuty(), borderCheck(data.borderCheck, data.coords))

    if not isJobValid(data.jobs) then return end
    if not IsOnDuty() then return end
    if not borderCheck(data.borderCheck, data.coords) then return end
...
-- EventHandlers
RegisterNetEvent("esx:setJob", setupDispatch)

AddEventHandler('esx:playerLoaded', setupDispatch)

AddEventHandler('esx:onPlayerUnloaded', removeZones)
RegisterNetEvent('ps-dispatch:refreshDispatch', setupDispatch)
