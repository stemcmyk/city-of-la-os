...
function GetIsHandcuffed()
    return exports.wasabi_police:IsHandcuffed()
end

function IsOnDuty()
    if Config.OnDutyOnly then
        if PlayerData.metadata.onduty == 1 then
            return true
        else
            return false
        end
...
function IsCallAllowed(message)
    ...
    if Config.PhoneRequired and not HasPhone() then lib.notify({description = 'You need a communications device for this.', type = 'error', duration = 5000}) return false end
