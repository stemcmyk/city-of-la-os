...
local function recursiveMerge(t1, t2)
    for k, v in pairs(t2) do
        if type(v) == "table" and type(t1[k] or false) == "table" then
            recursiveMerge(t1[k], t2[k])
        else
            t1[k] = v
        end
    end
...
local function addCustomBlacklist(config) 
    Config.Blacklist = recursiveMerge(Config.Blacklist, config)
end
exports("addCustomBlacklist", addCustomBlacklist)
    ...
    for i = 1, #components do
        ...
        for j = 1, #appliedComponents do
            ...
            if applied.component_id == componentId then
                ...
                exports.stem_gangsystem:checkClothingBlacklist(applied)
            end
        end
    end
