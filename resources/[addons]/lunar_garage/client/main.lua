...
function SpawnVehicle(args)
    ...
    exports.wasabi_carlock:GiveKey(props.plate)
    exports["esx-sna-fuel"]:SetFuel(vehicle, props.fuelLevel or 100.0)
    ...
    for _, vehicle in ipairs(vehicles) do
        ...
        local option = {
            ...
            onSelect = vehicle.state == 'in_garage' and SpawnVehicle or function()
                ...
                elseif vehicle.state == 'in_insurance' then
                    ShowNotification(locale('in_insurance_message'), 'error')
                end
            end
        }
...
local function saveVehicle(vehicle, free)
    ...
    local vehicleBodyDamage = 1000 - GetVehicleBodyHealth(vehicle)
    local vehicleEngineDamage = 1000 - GetVehicleEngineHealth(vehicle)    

    local vehicleRepairPrice = math.ceil((vehicleBodyDamage) + vehicleEngineDamage)

    if GetPedInVehicleSeat(vehicle, -1) ~= PlayerPedId() then
        ESX.ShowNotification("You need to be inside the vehicle to store.")
        return
    end

    if (vehicleRepairPrice > 50 and not free) then
        local input = lib.alertDialog({
            header = "Your vehicle is damaged",
            content = string.format("Would you like to repair the vehicle for **$%s**?", ESX.Math.GroupDigits(vehicleRepairPrice)),
            cancel = true,
            labels = {
                cancel = "No",
                confirm = "Yes"
        ...
        local paid = lib.callback.await('lunar_garage:payForRepair', false, vehicleRepairPrice)
        if (not paid) then
            ESX.ShowNotification("You cannot afford to store your vehicle in this garage.", true)
        end
        ...
        if not success then
            return ESX.ShowNotification("Repair cancelled")
        end
        
        ESX.ShowNotification(string.format("Paid **$%s** in repairs.", ESX.Math.GroupDigits(vehicleRepairPrice)), "success")
    end
    ...
    props.fuelLevel = exports["esx-sna-fuel"]:GetFuel(vehicle)
...
function refreshGarages () 
    for key, value in pairs(registeredZones) do
        value:remove()
    end
    for index, data in ipairs(Config.Garages) do
        if (not Config.Target or not data.PedPosition) and data.Position then
            local zone = lib.zones.sphere({
                ...
                radius = data.distance or Config.MaxDistance,
                onEnter = function()
                    if data.Jobs and not Utils.hasJobs(data.Jobs) then return end
                    if data.Users and not exports.stem_common:tableContains(data.Users, ESX.PlayerData.identifier) then return end
                ...
                },
                {
                    ...
                    onSelect = function()
                        ...
                        if Utils.distanceCheck(cache.ped, vehicle, 20.0) then
                            saveVehicle(vehicle, data.house)
                        end
                    end
                }
            })
...
TriggerServerEvent("lunar_garage:fetchGarages")
