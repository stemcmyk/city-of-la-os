...
function UpdateProspectingTargets(player)
    ...
    for _, target in next, PROSPECTING_TARGETS do
        ...
        targets[#targets + 1] = {target.x, target.y, target.z, difficulty, target.zoneSize}
    end
    TriggerClientEvent("prospecting:setTargetPool", player, targets)
end

function InsertProspectingTarget(resource, x, y, z, data, zoneSize)
    PROSPECTING_TARGETS[#PROSPECTING_TARGETS + 1] = {resource = resource, data = data, x = x, y = y, z = z, zoneSize = zoneSize}
end

function InsertProspectingTargets(resource, targets)
    for _, target in next, targets do
        InsertProspectingTarget(resource, target.x, target.y, target.z, target.data, target.zoneSize)
    end
end
...
AddEventHandler("prospecting:userRequestsLocations", function()
    ...
    TriggerClientEvent("prospecting:registerZones", -1, {{
        coords = currentZone.coords, 
        data = currentZone.data, 
        zoneSize = currentZone.zoneSize, 
        zoneLocations = currentZone.zoneLocations
   } })
end)
...
CreateThread(function()
    currentZone = Config.Zones[math.random(#Config.Zones)]
    GenerateCoords(currentZone.coords, currentZone.data, currentZone.zoneSize, currentZone.zoneLocations)
end)
...
function GenerateCoords(coords, data, zoneSize, zoneLocations)
	...
	while totalLocationsForOneCoord > 0 do
		...
		coordslist[#coordslist + 1] = {x = coordX, y = coordY, z = coords.z, data = data, zoneSize = zoneSize}
	end
