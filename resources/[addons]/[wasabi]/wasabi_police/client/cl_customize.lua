...
RegisterNetEvent('wasabi_police:sendToJail', function()
    ...
    elseif Config.Jail.jail == 'custom' then
        if (lib.callback.await('wasabi_police:isCuffed', 100, target)) then handcuffPlayer(target, "soft") end
...
function OpenPersonalStash(station)
    ...
    TriggerServerEvent('inventory:server:OpenInventory', 'stash',
        station .. '_' .. wsb.playerData.citizenid or wsb.playerData.identifier)
    TriggerEvent('inventory:client:SetCurrentStash', station .. '_' .. wsb.playerData.citizenid or wsb.playerData.identifier)
end
...
openJobMenu = function()
    ...
    Options[#Options + 1] = {
        ...
        description = "Impound a nearby vehicle",
        icon = 'car',
        onSelect = function ()
