...
RegisterNetEvent("wasabi_police:handcuffSelf")
AddEventHandler("wasabi_police:handcuffSelf", function (type) 
    local approved = IsTriggerApproved(GetInvokingResource())
    if not approved then return print("unapproved self cuff") end

    local playerId = GetPlayerServerId(PlayerId())

    print("register self cuff", playerId, type)

    handcuffPlayer(playerId, type)
end)
...
CreateThread(function()
    ...
    while true do
        ...
        if escorting?.active then
            ...
            if not textUI then
                wsb.showTextUI(string.format(Strings.stop_escorting_interact, Config.escort.stophotkey or "E"))
                textUI = true
            end
            ...
            if IsControlJustReleased(0, GetKeyNumber(Config.escort.stophotkey) or 38) or IsDisabledControlJustReleased(0, GetKeyNumber(Config.escort.stophotkey) or 38) or IsPedRagdoll(cache.ped)  then
                alrEscorting = nil
                escorting.active = nil
                ClearPedTasks(cache.ped)
                TriggerServerEvent('wasabi_police:releasePlayer', escorting.target)
