...
function IsVehicleAmbulance(entity) 
    return false;
end
                ...
                wsb.target.boxZone(k .. '_emscheckin', v.CheckIn.Target.coords, v.CheckIn.Target.width,
                    v.CheckIn.Target.length, {
                        ...
                        options = {
                            ...
                            {
                                name = k .. 'ems_checkinnearby',
                                event = 'wasabi_ambulance:attemptCheckinNearby',
                                icon = 'fa-solid fa-person',
                                distance = v.CheckIn.Target.distance,
                                label = "Check In Nearby Player",
                                hospital = k
                            },
                            {
                                name = k .. 'ems_purchaseBandage',
                                serverEvent = 'wasabi_ambulance:purchaseBandage',
                                icon = 'fa-solid fa-bandage',
                                distance = v.CheckIn.Target.distance,
                                label = "Buy bandages ($250)",
                                hospital = k
                            },
                        }
                    })
...
RegisterNetEvent('wasabi_ambulance:revive', function(noAnim)
    ...
    if wsb.framework == 'esx' then
        ...
        TriggerEvent("nearest-postal:resetUI")
    elseif wsb.framework == 'qb' then
        TriggerServerEvent('hospital:server:resetHungerThirst') -- qb-ambulancejob compatibility
        TriggerServerEvent('hud:server:RelieveStress', 100)
...
exports("getCheckInTimer", function () 
    return checkInTimer
end)

RegisterNetEvent("wasabi_ambulance:nearbyCheckinRevive", function (hospital) 
    TriggerEvent("nearest-postal:resetUI")
    ...
    TriggerServerEvent('wasabi_ambulance:tryRevive', hospital)
end)

exports("isPlayerDead", isPlayerDead)

AddEventHandler("wasabi_ambulance:attemptCheckinNearby", function(data) 
    ...
    for i = 1, #pool do
        ...
        if DoesEntityExist(ped) and IsPedAPlayer(ped) and exports.wasabi_ambulance:isPlayerDead(playerServerId) and IsEntityVisible(ped) and ped ~= PlayerPedId() then
    ...
    if closestPed then
        ...
        if player ~= PlayerId() then
            TriggerServerEvent('wasabi_ambulance:checkInPlayer', GetPlayerServerId(player), data.hospital)
        end
    else
        lib.notify({
            type = "error",
            title = "No player nearby to check in."
        })
    end
end)
