		...
		while bleedoutTimer > 0 and isDead do
			...
			if not IsCheckedIn then
				if IsControlPressed(0, 38) and timeHeld > 60 then
					HideDeathNui()
					if (not LocalPlayer.state.inHostileZone) then
						TriggerServerEvent("stem_common:nlr:registerDeath")
					end
					StartRPDeath()
					break
				end
...
RegisterNetEvent("esx:onPlayerDeath", function () 
    local lastShotTime = LocalPlayer.state.lastShotTime
	if not lastShotTime then return end
    if (GetGameTimer() < lastShotTime + (60 * 1000)) then
        LocalPlayer.state:set("nlrOnRespawn", "dead", true)
    end
end)
