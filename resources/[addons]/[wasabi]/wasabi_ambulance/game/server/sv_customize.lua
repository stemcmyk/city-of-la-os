...
RegisterNetEvent("wasabi_ambulance:purchaseBandage", function() 
    local player = ESX.GetPlayerFromId(source)
    if (player.getAccount("bank").money <= 250) then
        return player.showNotification("You do not have enough money for this.")
    end
    if (not player.canCarryItem("bandage", 1)) then
        return player.showNotification("You do not have enough space")
    end

    player.removeAccountMoney("bank", 250)
...
RegisterNetEvent('wasabi_ambulance:removeItemsOnDeath', function()
    ...
    elseif wsb.framework == 'esx' then
        -- player.setAccountMoney('money', 0) dont remove money
    ...
    elseif Config.Inventory == 'ox' then
        if (not GlobalState.purgeActive) then
            exports.ox_inventory:ClearInventory(src, { "phone", "driverlicense", "boatinglicense" })
        end
...
RegisterNetEvent("wasabi_ambulance:checkInPlayer", function (player, hospital) 
	-- if (GetResourceState("gay_baby_jail") ~= "missing") then
	...
	-- 		player --[[ integer ]],
	-- 		"Client" --[[ string ]],
	-- 		"BypassInvisible" --[[ string ]],
	-- 		true --[[ boolean ]],
	-- 		false --[[ boolean ]]
	-- 	)
	-- end
    TriggerClientEvent("FirstAid:CheckIn", player, hospital)
end)

RegisterNetEvent("wasabi_ambulance:hospitalResult", function(hospital, result)
    if (result) then 
        TriggerClientEvent('wasabi_ambulance:nearbyCheckinRevive', source, hospital)
    else
        TriggerEvent("stem_common:nlr:registerDeath", source)
    end
