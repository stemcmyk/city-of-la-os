    ...
    function()
        while true do
            ...
            if IsPedInAnyVehicle(ped, true) == false and lastTruck ~= 0 then
                ...
                if GetDistanceBetweenCoords(pos, markerCoords) < 5 then
                    ...
                    if GetDistanceBetweenCoords(markerCoords, pos) < 2 then
                        if IsControlJustPressed(0, config.extendKey) or IsControlJustPressed(0, config.retractKey) then
                            if trucks[lastTruck] ~= nil and trucks[lastTruck]["bedslide2"] ~= nil then
                                start = trucks[lastTruck]["bedslide2"]
                            end
                        end
                        if IsControlPressed(0,  config.extendKey) then
                            bedslide2("extend")
                        end
                        if IsControlJustReleased(0,  config.extendKey) or IsControlJustReleased(0, config.retractKey) then
                            -- TriggerServerEvent("saveArmPosition", lastTruck, start)
                        end
                        if IsControlPressed(0, config.retractKey) and start ~= 0.0 then
                            bedslide2("retract")
                        end
                    end
                end
            elseif has_value(vehs, GetEntityModel(veh)) and lastTruck ~= 0 then
                if
                    IsControlJustPressed(0,  config.extendKey) or IsControlJustPressed(0, config.retractKey) or IsControlJustPressed(0, 21) or
                ...
                if IsControlPressed(0, config.extendKey) or IsControlPressed(0, 21) then
                    bedslide("extend")
                end
                if IsControlJustReleased(0, config.extendKey) or IsControlJustReleased(0, config.retractKey) then
                    -- TriggerServerEvent("saveArmPosition", lastTruck, start)
                end
                if (IsControlPressed(0, config.retractKey) or IsControlPressed(0, 36)) and start ~= 0.0 then
                    bedslide("retract")
                end
            end
            Wait(0)
...
Citizen.CreateThread(
    function()
        while true do
            ...
            local rayHandle =
                CastRayPointToPoint(pos.x, pos.y, pos.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, lastVeh, -1)
            ...
            if IsPedInAnyVehicle(ped, false) then
                if IsEntityAttached(veh) then
                    ...
                    if IsControlJustReleased(0, config.carAttach) then
                        if vehicleHandle ~= nil and has_value(vehs, GetEntityModel(vehicleHandle)) then
                            local boneIndex = GetEntityBoneIndexByName(vehicleHandle, "misc_b")
                            ...
                            AttachEntityToEntity(
                                ...
                                boneIndex,
                                0,
                                (vehicleHeightMax.y - vehicleHeightMin.y) / 5.0,
                                math.abs(vehicleHeightMin.z),
                                2.0,
                                0,
                                0,
                                1,
