	...
	local options = {
		{
			...
			canInteract = function(entity)
                ...
                if not exports["wasabi_carlock"]:HasKey(GetVehicleNumberPlateText(entity)) then return false end

                local entityModel = GetEntityModel(entity)
                if (entityModel == GetHashKey("e450ambo") or entityModel == GetHashKey("stretcher")) then
                    return false
                end
