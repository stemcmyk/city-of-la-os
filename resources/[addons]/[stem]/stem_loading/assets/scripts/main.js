...
function setSong(nSongIndex) {
  songIndex = nSongIndex
  let nsong = Config.Songs[songIndex];

  if (song) song.pause()

  song = new Audio("assets/media/music/" + nsong.file);
  song.volume = 0.2

  $("#track-play").data("active", true)
  $("#track-pause").data("active", false)

  $("#track-info .track-name").html(nsong.label)
...
function setup() {
  const body = $("body");
  const audio = $(`<audio id="music">`);
  body.append(audio);

  Config.Songs.map((song) => {
    console.log(song.file)
    var source = $(
      `<source src='assets/media/music/${song.file}' type="audio/mpeg">`
    );
    audio.append(source);
  });
  ...
  // Music
  setSong(Math.floor(Math.random() * Config.Songs.length))

  // Categories
  var currentCat = "";
  Config.Categories.forEach((cat, index) => {
    $(".categories .buttons").append(
      `<p data-category="${index}" class="${cat.default ? "active" : ""}">${cat.label
      }</p>`
    );
  ...
  $(".categories .buttons p").on("click", function () {
    $(`.categories .buttons p[data-category="${currentCat}"]`).removeClass(
      "active"
    );
    currentCat = $(this).attr("data-category");
    $(`.categories .buttons p[data-category="${currentCat}"]`).addClass(
      "active"
    );
  ...
  $("#track-back").on("click", function () {
    songIndex = songIndex - 1
    if (!Config.Songs[songIndex]) {
      songIndex = Config.Songs.lenght - 1
    }
    setSong(songIndex)
  })

  $("#track-play").on("click", function () {
    $("#track-play").addClass("hidden")
    $("#track-pause").removeClass("hidden")
    song.play()
  })

  $("#track-pause").on("click", function () {
    $("#track-pause").addClass("hidden")
    $("#track-play").removeClass("hidden")
    song.pause()
  })

  // Socials
  Config.Socials.forEach((social, index) => {
    $(".categories .socialmedia").append(
      `<div class="box" data-id="${social.name}" data-link="${social.link}"><img class="icon" src="${social.icon}"><div class="info"><p class="title">${social.label}</p><p class="description">${social.description}</p></div></div>`
    );
  });
  ...
  $(".staff .next").on("click", function () {
    if (currentPage < Config.Staff.length - 2) {
      ...
      $(".staff .innercards").css(
        "transform",
        `translate3d(calc(-${currentPage * 50}% - ${(currentPage + 1) * 0.5
        }vw), 0, 0)`
      );
    }
  });
...
$(".loader .filled-logo").css("height", 50 + "%");
$(".loader .progress").html(50 + "%");
