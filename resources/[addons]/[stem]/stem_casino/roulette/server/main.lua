...
function getPlayerChips(source)
    ...
    --     else end
    -- else end
    return getChipsCallback(source) or 0
end

function giveChips(source, amount)
    giveChipsCallback(source, amount)
end 

function removeChips(source, amount)
    takeChipsCallback(source, amount)
end
...
ESX.RegisterServerCallback('roulette:server:ChipsAmount', function(source, cb)
    local retval = 0
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('server_remote:rulett:taskSitDown',function(rulettIndex, chairData)
    ...
    if aktivRulettek[rulettIndex] ~= nil then
        if aktivRulettek[rulettIndex].chairsUsed[chairId] ~= nil then
            return TriggerClientEvent('esx:showNotification', source, 'This chair is occupied.','error')
        else
...
RegisterNetEvent('casino:taskStartRoulette',function(rulettIndex, chairId)
    ...
    if aktivRulettek[rulettIndex] == nil then
        aktivRulettek[rulettIndex] = {
            statusz = false,
            ido = Config_roulette.RouletteStart,
            bets = {},
            chairsUsed = {}
        }

        Config_roulette.DebugMsg(string.format('created rulett on serverside. table: %s', rulettIndex))
    end
    if aktivRulettek[rulettIndex].chairsUsed[chairId] == nil then
        aktivRulettek[rulettIndex].chairsUsed[chairId] = source
        TriggerClientEvent('client:casino:openRulett', source, rulettIndex)
    else
        TriggerClientEvent('esx:showNotification', source, 'This chair is occupied.','error')
    end
end)
            ...
            for rulettIndex, v in pairs(aktivRulettek) do
                if v.statusz == false then
                    ...
                    if v.ido < 1 then
                        ...
                        if Config_roulette.TestTicker ~= nil then
                            randomSpinNumber = Config_roulette.TestTicker
                        end
                        local WinningBetIndex = Config_roulette.rouletteSzamok[randomSpinNumber]

                        Config_roulette.DebugMsg(string.format('Rulett randomSpinNumber: %s, which is number: %s', randomSpinNumber, WinningBetIndex))
                        ...
                        Citizen.CreateThread(
                            function()
                                Config_roulette.DebugMsg('time remaining 0, starting the spin events.')
                                ...
                                if #v.bets > 0 then
                                    ...
                                    aktivRulettek[rulettIndex].ido = Config_roulette.RouletteStart
                                    ...
                                    if countTablePlayers(rulettIndex) < 1 then
                                        ...
                                        Config_roulette.DebugMsg(string.format('Rulett table id %s, stopped because no one using it.', rulettIndex))
...
function CheckWinners(bets, WinningBetIndex)
    ...
    for i = 1, #bets, 1 do
        ...
        local player = ESX.GetPlayerFromId(targetSrc)
                ...
                exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
                    exports.stem_discord:sendThroughWebhook({
                        ...
                        params = {
                            ...
                            content = ("<@%s> ``%s (%s)`` has won **$%s** from **Roulette** "):format(playerDiscord, player.name, GetPlayerName(player.source), betData.betAmount)
                        }
                    })
                end)
            elseif
    ...
    for targetSrc, _ in pairs(playersLoss) do
        ...
        if chairId ~= nil then
            ...
            TriggerClientEvent('esx:showNotification', targetSrc, 'You Lost... Better luck next time', "error")
        end
    end
...
RegisterNetEvent('casino:taskBetRulett',function(rulettIndex, betId, betAmount)
    ...
    if aktivRulettek[rulettIndex] ~= nil then
        if aktivRulettek[rulettIndex].statusz then
            return TriggerClientEvent('esx:showNotification', src, 'The game started, you can not bet at the moment.', "error")
        end
        local chipsAmount = getPlayerChips(src)

        local totalBets = betAmount
        for key, value in pairs(aktivRulettek[rulettIndex].bets) do
            totalBets = totalBets + value.betAmount
        end

        local supporterAccess = Player(source).state.supporterAccess
        if (totalBets > 5000 and (not supporterAccess or not exports.stem_common:tableContains(supporterAccess, "emerald"))) then
            TriggerClientEvent("esx:showHelpNotification", source, "You need ~g~Emerald Supporter~w~ to place more bets", false, true)
            return
        end
        ...
        if chipsAmount ~= nil then
            if chipsAmount >= betAmount then 
                ...
                -- TriggerClientEvent('esx:showNotification', src, betAmount..' chips bet on ['..betId..']')
                Config_roulette.DebugMsg(string.format('player %s betted %s chips on betId: %s', GetPlayerName(src), betAmount, betId))
                local exist = false
                ...
                if chairId ~= nil then
                    Config_roulette.DebugMsg(string.format('%s chair betanim play', chairId))
                    TriggerClientEvent('client:rulett:playBetAnim', src, chairId)
                end
            else
                TriggerClientEvent('esx:showNotification', src, 'You do not have enough Casino chips to place bet.', "error")
            end
        else
            TriggerClientEvent('esx:showNotification', src, 'You dont have any caino chips', 'error')
        end
    else 
        TriggerClientEvent('esx:showNotification', src, 'error', 'An error occurred on a non-existent roulette table server side?')
    end
end)
