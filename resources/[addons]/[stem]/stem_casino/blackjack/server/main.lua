ranks = {'02', '03', '04', '05', '05', '06', '07', '07', '08', '09', '10', --[['11',]] 'JACK', 'QUEEN', 'KING', 'ACE'}
...
function GiveMoney(player, money)
	...
	exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
		exports.stem_discord:sendThroughWebhook({
			webhook = WEBHOOK,
			params = {
				username = "Blackjack Win",
				content = ("<@%s> ``%s (%s)`` has won **$%s** from **Blackjack**"):format(playerDiscord, player.name, GetPlayerName(player.source), ESX.Math.GroupDigits(money))
			}
		})
	end)
...
function TakeMoney(player, money)
	...
	exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
		exports.stem_discord:sendThroughWebhook({
			...
			params = {
				username = "Blackjack Win",
				content = ("<@%s> ``%s (%s)`` placed a **$%s** bet in **Blackjack**"):format(playerDiscord, player.name, GetPlayerName(player.source), ESX.Math.GroupDigits(money))
			}
		})
	end)
...
function CheckPlayerBet(i, bet, high)
	...
	if (high and (not supporterAccess or not exports.stem_common:tableContains(supporterAccess, "emerald"))) then
		TriggerClientEvent("esx:showHelpNotification", source, "You need ~g~Emeral Supporter~w~ to play the high ", false, true)
		canBet = false
	end
