                ...
                if not blips_cache[follow_id_key].needs_server and not blips_cache[follow_id_key].needs_removal then
                    if not blips_cache[follow_id_key].synced_blip_id then
                        ...
                        if DoesEntityExist(blips_cache[follow_id_key].player_ped) then
                            ...
                            doDebug("ERROR - Could not find networked player ped")
                        end
                    end

                    if DoesEntityExist(data.player_ped) and DoesBlipExist(GetBlipFromEntity(data.player_ped)) then
                        ...
                        if veh ~= 0 then
                            ...
                            local isBike = config.customVehicleTypes[GetEntityModel(veh)] == "bike" or GetVehicleClass(veh) == 8
                            ...
                            SetBlipRotation(blip_id, math.ceil(rotation))
                        end
                    else
                        if DoesBlipExist(blips_cache[follow_id_key].blip_id) then
                            setBlipProperties(blips_cache[follow_id_key].blip_id, data.properties)
                        end

                        doDebug("Blip is not networked - updating blip anyways")
                    end
                end
            end
        end
...
function setBlipProperties(blip_id, properties)
    ...
    local in_vehicle = properties[8]
    local siren_enabled = properties[9]
    local isBoat = properties[10]
    local isHeli = properties[11]
    local isBike = properties[12]

    local blipType = config.default_type._type
    ...
    local has_vehicle_blip = blip_types[template_type]._vehicle_blip_type
    local siren_colors = blip_types[template_type]._vehicle_siren_colors
    local display_type = blip_types[template_type].display_type

    if blip_types[template_type]._type then
        blipType = blip_types[template_type]._type
    end
    if blip_types[template_type]._color then
        color = blip_types[template_type]._color
    end
    if blip_types[template_type]._scale then
        scale = blip_types[template_type]._scale
    end
    if blip_types[template_type]._alpha then
        alpha = blip_types[template_type]._alpha
    end
    if blip_types[template_type]._show_local_direction then
        show_local_indicator = blip_types[template_type]._show_local_direction
    end
    if blip_types[template_type]._show_off_screen then
        show_off_screen = blip_types[template_type]._show_off_screen
    end
    if current_type ~= blipType or current_color ~= color or has_vehicle_blip then
        if in_vehicle then
            if blip_types[template_type]._vehicle_blip_type then
                SetBlipSprite(blip_id, blip_types[template_type]._vehicle_blip_type)
            end
            if blip_types[template_type]._vehicle_blip_scale then
                SetBlipScale(blip_id, blip_types[template_type]._vehicle_blip_scale)
            else
                SetBlipScale(blip_id, scale)
            end

            if (siren_colors and siren_enabled) and not isHeli then
                if current_color ~= siren_colors[1] then
                    SetBlipColour(blip_id, siren_colors[1])
                else
                    SetBlipColour(blip_id, siren_colors[2])
                end
            else
                SetBlipColour(blip_id, color)
            end
        else
            SetBlipSprite(blip_id, blipType)
            SetBlipScale(blip_id, scale)
            SetBlipColour(blip_id, color)
        end

        if isBoat then
            SetBlipSprite(blip_id, config.default_type._boat_blip_type)
            SetBlipAsShortRange(blip_id, false)
        elseif isHeli then
            SetBlipSprite(blip_id, config.default_type._heli_blip_type)
            SetBlipAsShortRange(blip_id, false)
            SetBlipColour(blip_id, color)
        elseif isBike then
            SetBlipSprite(blip_id, config.default_type._bike_blip_type)
        ...
        if display_type ~= nil then
            SetBlipDisplay(blip_id, display_type)
        end

        -- SetBlipColour(blip_id, color)
        SetBlipAlpha(blip_id, alpha)
        -- SetBlipAsShortRange(blip_id, not show_off_screen)
        SetBlipRotation(blip_id, math.ceil(properties[7]))

        -- if show_local_indicator then
        --     ShowHeadingIndicatorOnBlip(blip_id, true)
        -- end

        -- if in_vehicle and blip_types[template_type]._vehicle_blip_cone then
        --     SetBlipShowCone(blip_id, true)
        -- else
        --     SetBlipShowCone(blip_id, false)
        -- end
...
ESX.RegisterClientCallback("stem_common:blips:isInVehicle", function (cb) 
    local ped = PlayerPedId()
    local vehicle = GetVehiclePedIsIn(ped, false)
    cb(IsPedInAnyVehicle(ped, true), IsVehicleSirenOn(vehicle), GetVehicleClass(vehicle), GetEntityModel(vehicle))
end)