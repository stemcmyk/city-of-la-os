...
--[[
	...
	*  Add the value to position one of the original table and return it
	...
	if not t then
		print("fsn_main (cl_utils.lua) [Util.PrependTable] - You need to provide a table in argument position 1")
		return
	end
	if not v then
		print("fsn_main (cl_utils.lua) [Util.PrependTable] - You need to provide a value in argument position 2")
		return
	end
	if #t > 0 and not t[1] then
		print("fsn_main (cl_utils.lua) [Util.PrependTable] - Table provided is not using numbers for keys.")
		return
	end
...
function Util.GetVecDist(v1, v2)
	...
	return math.sqrt(
		((v1.x or 0) - (v2.x or 0)) * ((v1.x or 0) - (v2.x or 0)) + ((v1.y or 0) - (v2.y or 0)) * ((v1.y or 0) - (v2.y or 0)) +
