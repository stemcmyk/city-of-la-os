...
RegisterNetEvent("pixel_anticl:combatLogSpawn", function (departement)
    while not LocalPlayer.state.switchComplete do
        Wait(50)
    end

    local coords = Config.combatLogLocations[departement]
    
    DoScreenFadeOut(500)
    Citizen.Wait(500)
    SetEntityCoords(PlayerPedId(), coords.x, coords.y, coords.z, true, false)
    SetEntityHeading(coords.w)
    DoScreenFadeIn(500)

    lib.notify({
        type = "warning",
        title = "Previously Arrested",
        description = string.format("You we're previously arrested by **%s** and therefor been sent to their jail.", string.upper(departement))
    })
end)
