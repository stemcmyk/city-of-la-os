...
LocalPlayer.state.garbageRouteId = nil
LocalPlayer.state.garbageInService = nil
...
function StartInService()
    ...
    if InService then 
        ...
        if not ESX.Game.IsSpawnPointClear(Shared.SpawnVehicle, 3.0) then
            ESX.ShowNotification(Shared.Locale.VehicleNear, "error")
        else
            ESX.Game.SpawnVehicle(model, Shared.SpawnVehicle, 270.0, function(vehicle)
                Vehicle = vehicle
                local plate = 'GARB-'..math.random(0,2000)
                SetVehicleNumberPlateText(vehicle, plate)
                exports["wasabi_carlock"]:GiveKey(plate)
                TriggerServerEvent("stem_garbage:beginService", NetworkGetNetworkIdFromEntity(vehicle))
            end)
        end
        InGarage = true 
    elseif not InService then
        TriggerServerEvent("stem_garbage:finishService")
        InGarage = false
    end 
end
...
local function SetBag(entity, Aria)
    ...
    if not Aria then
        ...
        TriggerServerEvent("stem_garbage:beginRoute")
...
Citizen.CreateThread(function()
    while true do
        ...
        if InService then
            ...
            if not LocalPlayer.state.garbageRouteId then
                if not Route then
                    Route = {}
                    Garage = {}
                    ...
                    for bag, data in pairs(Route) do
                        if Shared.GarbagePos[bag] and GetDistanceBetweenCoords(pCoords, Shared.GarbagePos[bag], true) < 32 then
                            LocalPlayer.state:set("garbageRouteId", bag)
            ...
            elseif LocalPlayer.state.garbageRouteId then
                local BagPos = Shared.GarbagePos[LocalPlayer.state.garbageRouteId]
                        ...
                        if IsControlJustPressed(1, 51) and not IsInAction then
                            ...
                            if (GetDistanceBetweenCoords(pCoords, GetEntityCoords(vehicle) - GetEntityForwardVector(vehicle) * 5) < 2 or IsPedInAnyVehicle(pPed)) and GetEntityModel(vehicle) == 1917016601 then
                                ...
                                Citizen.CreateThread(function()
                                    ...
                                    TriggerServerEvent("stem_garbage:updateRoute", NetworkGetNetworkIdFromEntity(vehicle))
        ...
        elseif LocalPlayer.state.garbageRouteId or (IsDutty and GetSelectedPedWeapon(pPed) ~= HashBag) or Route then
            IsInAction = false
