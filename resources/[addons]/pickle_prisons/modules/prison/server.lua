...
function GiveRewards(source, rewards)
    for i=1, #rewards do 
        ...
        elseif Config.XPEnabled and reward.type == "xp" then 
            local stats = Player(source).state.stats
            stats.exercises_completed = (stats.exercises_completed ~= nil and stats.exercises_completed or 0) + 1

            Player(source).state:set("stats", stats, true)
        end
    end
end
...
function JailPlayer(from, source, time, index, noSave)
    ...
    local jailer = ESX.GetPlayerFromId(from)
    ...
    exports.stem_discord:sendMessage({
        ...
        content = string.format("``%s`` Jailed ``%s`` for **%s months**", jailer and jailer.name or "Server", player.name, time)
    })
...
lib.cron.new("* * * * *", function (task, date)
    for source, value in pairs(Prisoners) do
        local player = ESX.GetPlayerFromId(source)
...
function UnjailPlayer(source, breakout, court)
    ...
    -- end
    TriggerClientEvent("pickle_prisons:unjailPlayer", source, data, court)
end
...
RegisterNetEvent("pickle_prisons:jailPlayer", function(target, time, index)
    local source = source
    Player(source).state.handcuffed = false
...
function JailEvent(source, target, time, index)
    ...
    if prisoner then 
        ...
        if not CheckPermission(source, permissions.jail) then 
            ...
            Player(source).state.handcuffed = false;
            JailPlayer(source, target, time, index)
        end
    end
end
...
function UnjailEvent(source, target)
    ...
    if not prisoner then 
        if (not source) then
            return print("Prison: Invalid Unjail Target")
        end
        ...
        local prison = Config.Prisons[index]
        local permissions = prison.permissions or Config.Default.permissions 
        if source and not CheckPermission(source, permissions.unjail) then 
            ShowNotification(source, _L("no_permission"))
        else
            local unjailer = source and ESX.GetPlayerFromId(source) or nil
            local player = ESX.GetPlayerFromId(target)
            exports.stem_discord:sendMessage({
                channelId = "1237768213711028276",
                content = string.format("``%s`` Unjailed ``%s``", unjailer and unjailer.name or "Server", player.name)
            })
...
ESX.RegisterCommand("unjail", { "admin" }, function(player, args, raw)
    UnjailEvent(player and player.source or false, args.player)
end, true, {
    arguments = {
		{ name = 'player', help = 'The player id', type = 'number' }
    }
})
