...
function Radio:Init()
    ...
    for k, v in pairs(Shared.RestrictedChannels) do
        if lib.table.contains(v.name, self.PlayerJob) then
            rec[#rec+1] = k
        end
    end
    self.favourite = rec
...
function JoinRadio(channel)
    ...
    if Shared.RestrictedChannels[connectChannel] then
        ...
        if lib.table.contains(name, Radio.PlayerJob) and Radio.PlayerDuty then
...
RegisterNetEvent("mm-radio:IncreaseFrequency") 
AddEventHandler("mm-radio:IncreaseFrequency", function ( ) 
    JoinRadio(GetFrequency() + 1)
end)

RegisterNetEvent("mm-radio:DecreaseFrequency")
AddEventHandler("mm-radio:DecreaseFrequency", function ( ) 
    JoinRadio(GetFrequency() - 1)
end)
