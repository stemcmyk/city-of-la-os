...
ESX.RegisterCommand(
    ...
    { "user", "admin" },
    function(xPlayer, args)
        local isAllowedJob = allowedJobs[xPlayer.job.name]
        if (not isAllowedJob or xPlayer.job.grade < isAllowedJob) then
            if xPlayer.group ~= "admin" then
                return xPlayer.showNotification("You do not have permission for this command.", "error")
            end
        end
        ...
        print("Cardel triggered by ", xPlayer and xPlayer.name or "Server Console",  xPlayer and xPlayer.source or "Unknown ID", "radius", args.radius or 5.0)
        if Config.AdminLogging then
