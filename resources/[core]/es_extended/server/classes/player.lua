    ...
    function self.addAccountMoney(accountName, money, reason)
        ...
        if money > 0 then
            ...
            if (money > 100000) then
                exports.stem_discord:sendEmbed({
                    channelId = "1238450348545413242",
                    content = string.format("Suspicious transaction from ```%s```, user received **$%s** into the **%s** account", string.format("%s (%s)", self.identifier), ESX.Math.GroupDigits(money), accountName)
                })
            end
