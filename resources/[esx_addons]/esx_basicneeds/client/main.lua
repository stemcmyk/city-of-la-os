...
AddEventHandler('esx_basicneeds:onUse', function(food_type, prop_name, anim)
	if not IsAnimated then
		...
		if type(prop_name) == "table" then
			propObject = prop_name
			prop_name = propObject.name
		end

		if food_type == 'food' then
			anim = anim
		elseif food_type == 'drink' then
			anim = anim
		end
		...
		CreateThread(function()
			...
			if (prop_name) then
				...
				AttachEntityToEntity(prop, playerPed, boneIndex, propObject.pos or vector3(0.12, 0.028, 0.001), propObject.rot or vector3(10.0, 175.0, 0.0), true, true, false, true, 1, true)
			end
