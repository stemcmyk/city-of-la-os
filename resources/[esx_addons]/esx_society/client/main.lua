function OpenBossMenu(society, close, options)
	...
	ESX.TriggerServerCallback('esx_society:isBoss', function(isBoss)
		if isBoss then
			...
			if options.posTerminal then
				elements[#elements+1] = {icon = "fas fa-wallet", title = "Buy Payment Terminal ($250)", value = "purchase_pos_terminal"}
			end

			ESX.OpenContext("right", elements, function(menu,element)
				if element.value == "check_society_balance" then
					TriggerServerEvent('esx_society:checkSocietyBalance', society)
				elseif (element.value == "purchase_pos_terminal") then
					local dialogResponse = lib.inputDialog("Purchase Terminals", { 
						{ 
							label = "Total Terminals", 
							type = "number",
						}, 
						{ 
							label = "Player Payment Ratio", 
							type = "slider", 
					...
					if (not dialogResponse) then return end

					local totalPrice = tonumber(dialogResponse[1]) * 250

					local confirmDialog = lib.alertDialog({
						content = ("Are you sure you want to purchase %s for **$%s**"):format(dialogResponse[1], ESX.Math.GroupDigits(totalPrice)),
						header = "Are you sure?",
						cancel = true,
						centered = true,
					})

					if (confirmDialog == "cancel") then return end

					TriggerServerEvent("stem_common:payment_terminal:purchaseTerminal", tonumber(dialogResponse[1]), society, dialogResponse[2])
