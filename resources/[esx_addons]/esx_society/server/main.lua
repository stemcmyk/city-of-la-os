...
local SOCIETY_WEBHOOK = GetConvar("society_webhook", nil)
...
AddEventHandler('esx_society:withdrawMoney', function(societyName, amount)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
		if amount > 0 and account.money >= amount then
			...
			if (SOCIETY_WEBHOOK) then
				exports.stem_discord:sendThroughWebhook({
					webhook = SOCIETY_WEBHOOK,
					params = {
						username = "Society Logs",
						content = ("``%s`` %s **%s** withdrew **$%s** from **%s** (New Total Balance: **$%s**)"):format(xPlayer.identifier, xPlayer.job.grade_label, xPlayer.name, ESX.Math.GroupDigits(amount), society.account, ESX.Math.GroupDigits(account.money - amount)),
					},
					ignoreEnv = true,
				})
			else
				print("No society_webook found")
			end
		...
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			...
			if (SOCIETY_WEBHOOK) then
				exports.stem_discord:sendThroughWebhook({
					...
					params = {
						username = "Society Logs",
						content = ("``%s`` %s  **%s** deposited **$%s** into **%s** (New Total Balance: **$%s**)"):format(xPlayer.identifier, xPlayer.job.grade_label, xPlayer.name, ESX.Math.GroupDigits(amount), society.account, ESX.Math.GroupDigits(account.money + amount)),
					},
					ignoreEnv = true,
				})
			else
...
exports("setBossGradeName", function (grade_name)
	Config.BossGrades[grade_name] = true
end)
