...
local function AddLicense(identifier, licenseType, cb)
	MySQL.insert('INSERT INTO user_licenses (type, owner, issuedate) VALUES (?, ?, ?)', {licenseType, identifier, os.date('%Y-%m-%d %H:%M:%S', os.time())}, function(rowsChanged)
		if cb then
			cb(rowsChanged)
		end
	end)
...
RegisterCommand("addLicense", function (src) 
	...
	AddLicense(player.identifier, "drive")
end)
