...
lib.addCommand("replacePerms", {
	help = "Replace a perm for every door",
	params = {
		{
			name = "from",
			help = "From Perm",
	...
	local player = GetPlayer(source)
	if (not args.from or not args.to) then return player.showNotification("invalid arguments", "error") end


	MySQL.Async.fetchAll("SELECT * FROM ox_doorlock", {}, function (result)
		if (not result or #result <= 0) then return error("fuck") end
		local updatedDoors = {}
		for index, column in ipairs(result) do
			local door = json.decode(column.data)
			local updated = false;
			if (door.groups) then
				local newGroups = {}
				for key, value in pairs(door.groups) do
					if (key == args.from) then
						updated = true
						newGroups[args.to] = value
					else
						newGroups[key] = value
					end
				end
				door.groups = newGroups
			end
			if (updated) then  
				column.data = door;
				table.insert(updatedDoors, column)
			end;
		end
		for index, door in ipairs(updatedDoors) do
			MySQL.Async.execute("UPDATE ox_doorlock SET data = @data WHERE id = @id", {
				["@id"] = door.id,
				["@data"] = json.encode(door.data),
			}, function(result)
				createDoor(door.id, door.data, door.name)
				TriggerClientEvent("ox_doorlock:setState", -1, door.id, door.data.state, player.source, door.data)
				if (door.id == #updatedDoors) then
					player.showNotification(string.format("Updated %s doors", #updatedDoors), "success")
					lib.callback.await("ox_doorlock:updateDoors", -1 , doors)
				end
			end)
		end
	end)
