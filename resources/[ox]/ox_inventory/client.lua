...
local function canOpenTarget(ped)
	...
	or exports.wasabi_ambulance:isPlayerDead(GetPlayerServerId(NetworkGetPlayerIndexFromPed(ped)))
...
function client.openInventory(inv, data)
	...
	if canOpenInventory() then
		...
		if inv == 'player' and data ~= cache.serverId then
			...
			if not targetCoords or #(targetCoords - GetEntityCoords(playerPed)) > 3 or not canOpenTarget(targetPed) then
				return lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access') })
			end
		end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	RegisterNetEvent("ox_inventory:boxBuyLicense:"..id)
	AddEventHandler("ox_inventory:boxBuyLicense:"..id, function () 
		lib.callback('ox_inventory:buyLicense', 1000, function(success, message)
			if success ~= nil then
				lib.notify({
					id = message,
		...
		exports.ox_target:addBoxZone({
			...
			options = {
				{
					event = "ox_inventory:boxBuyLicense:"..id,
					icon = "fas fa-gun",
					label = locale('purchase_license', data.name, "$"..exports.stem_common:formatPrice(data.price))
				},
			},
		})
		-- lib.points.new({
			...
			if not canOpenInventory() then
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					if currentInventory.type == 'otherplayer' then
						...
						if not id or #(playerCoords - pedCoords) > 3.0 or (not canOpenTarget(ped) and not Player(playerServerId).state.handsup) then
							client.closeInventory()
					...
					elseif currentInventory.coords and (#(playerCoords - currentInventory.coords) > (currentInventory.distance or 3.0)) then
						client.closeInventory()
						lib.notify({ id = 'inventory_lost_access', type = 'error', description = locale('inventory_lost_access') })
					end
				end
