...
function server.buyLicense(inv, license)
	...
	if (license.playtime and Player(inv.id).state.playTime < license.playtime) then
		return false, 'not_enough_playtime'
	end
	if server.hasLicense(inv, license.name) then
		return false, 'already_have'
	elseif player.getAccount("bank").money < license.price then
		return false, 'can_not_afford'
	end
	player.removeAccountMoney("money", license.price)
