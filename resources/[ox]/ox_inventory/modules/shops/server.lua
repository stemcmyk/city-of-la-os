...
function openShop(source, data)
	...
	if data then
		...
		if shop.coords and type(shop.coords) == 'vector3' and #(GetEntityCoords(GetPlayerPed(source)) - shop.coords) > 10 then
			return
		end
...
lib.callback.register('ox_inventory:openShop', openShop)
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local canAfford = player.getAccount("bank").money >= price
