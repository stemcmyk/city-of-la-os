...
local INVENTORY_WEBHOOK = GetConvar("inventory_webhook", nil)
...
local function loadInventoryData(data, player)
	...
	elseif data.type == 'policeevidence' then
		inventory = Inventory.Create(data.id, locale('police_evidence'), data.type, 100, 0, 100 * 1000 * 1000, false)
	else
...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData then
			...
			if (INVENTORY_WEBHOOK) then
				if (data.toType == "trunk" or data.toType == "glovebox") then
					local player = ESX.GetPlayerFromId(source)
					exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
						exports.stem_discord:sendThroughWebhook({
							webhook = INVENTORY_WEBHOOK,
							params = {
								username = "Vehicle Transfer Logs",
								content = ([[
									<@%s> ``%s`` **%s** transferred **x%s %s** to vehicle **%s**'s ``%s``
								]]):format(
									playerDiscord, player.identifier, player.name, 
									ESX.Math.GroupDigits(data.count), toData?.name, toInventory.label, data.fromType
								),
							},
							ignoreEnv = true,
						})
				...
				if (data.fromType == "trunk" or data.fromType == "glovebox") then
					local player = ESX.GetPlayerFromId(source)
					exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
						exports.stem_discord:sendThroughWebhook({
							webhook = INVENTORY_WEBHOOK,
							params = {
								username = "Vehicle Transfer Logs",
								content = ([[
									<@%s> ``%s`` **%s** transferred **x%s %s** from vehicle **%s**'s ``%s``
								]]):format(
									playerDiscord, player.identifier, player.name, 
									ESX.Math.GroupDigits(data.count), toData?.name, fromInventory.label, data.fromType
								),
							},
							ignoreEnv = true,
						})
				...
				print("No inventory_webhook found")
			end
...
RegisterServerEvent('ox_inventory:giveItem', function(slot, target, count)
	...
	if toInventory?.player then
		...
		local targetItemCount = targetPlayer.hasItem(data.name).count
		local playerItemCount = player.hasItem(data.name).count

		if (INVENTORY_WEBHOOK) then
			exports.stem_discord:getUserDiscord(targetPlayer.source, function (targetDiscord) 
				exports.stem_discord:getUserDiscord(player.source, function (playerDiscord) 
					if (data.name == "money" or data.name == "black_money") then
						exports.stem_discord:sendThroughWebhook({
							webhook = INVENTORY_WEBHOOK,
							params = {
								username = "Money Transfer Logs",
								content = ([[
									<@%s> ``%s`` **%s** transfered **$%s** %s to <@%s> ``%s`` **%s**
									(<@%s>) Balance Pre/Post: $%s / $%s |  (<@%s>) Balance Pre/Post: $%s / $%s
								]]):format(
									playerDiscord, player.identifier, player.name, 
									ESX.Math.GroupDigits(count), data.name, 
									targetDiscord, targetPlayer.identifier, targetPlayer.name,
									playerDiscord, ESX.Math.GroupDigits(playerItemCount), ESX.Math.GroupDigits(playerItemCount - count),
									targetDiscord, ESX.Math.GroupDigits(targetItemCount), ESX.Math.GroupDigits(targetItemCount + count)
								),
							},
							ignoreEnv = true,
						})
						...
						exports.stem_discord:sendThroughWebhook({
							webhook = INVENTORY_WEBHOOK,
							params = {
								username = "Item Transfer Logs",
								content = ([[
									<@%s> ``%s`` **%s** transfered x%s **%s** to <@%s> ``$%s`` **%s**
									(<@%s>) Items Pre/Post: %s / %s |  (<@%s>) Items Pre/Post: %s / %s
								]]):format(
									playerDiscord, player.identifier, player.name, count, data.name, 
									targetDiscord, targetPlayer.identifier, targetPlayer.name,
									playerDiscord, ESX.Math.GroupDigits(playerItemCount), ESX.Math.GroupDigits(playerItemCount - count),
									targetDiscord, ESX.Math.GroupDigits(targetItemCount), ESX.Math.GroupDigits(targetItemCount + count)
								),
